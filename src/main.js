...
scene.background = new THREE.Color("#f9f4e1"); // Changed from purple to cream
...
manager.onLoad = function () {
  // Show the welcome message but make button non-interactive
  loadingScreenButton.textContent = "Welcome, Ka WFH PH";
  loadingScreenButton.style.border = "8px solid #8c5519"; // Changed from purple to brown
  loadingScreenButton.style.background = "#f9f4e1"; // Changed from purple to cream
  loadingScreenButton.style.color = "#543310"; // Changed from purple to brown
  loadingScreenButton.style.boxShadow = "none";
  loadingScreenButton.style.cursor = "default"; // Make it non-clickable
  loadingScreen.style.background = "#f9f4e1"; // Changed from purple to cream
  ...
  // Add a small delay to ensure everything is loaded, then start reveal
  setTimeout(() => {
    playReveal();
  }, 100);
};
  ...
  pianoKeys.forEach((key, index) => {
    pianoKeysTl
      .to(
        key.position,
        {
          y: key.userData.initialPosition.y + 0.1, // Smaller movement
          duration: 0.3, // Faster
          ease: "power2.out", // Less bouncy animation
        },
        index * 0.1
      )
      .to(
...
const glassMaterial = new THREE.MeshPhysicalMaterial({
  transmission: 1,
  opacity: 1,
  color: 0x3a2208, // Changed to darker brown
  metalness: 0,
  roughness: 0,
  ior: 3,
  thickness: 0.01,
  ...
  envMapIntensity: 1,
  depthWrite: false,
  specularColor: 0x3a2208, // Changed to darker brown
});

const whiteMaterial = new THREE.MeshBasicMaterial({
  color: 0xfff8e7, // Changed from pure white to very light cream
});
...
const createMaterialForTextureSet = (textureSet) => {
  const material = new THREE.ShaderMaterial({
    uniforms: {
      ...
      uMixRatio: { value: 1 }, // 1 = night mode
...
function hasIntroAnimation(objectName) {
  // Objects with _Fifth in their name should not have intro animations
  if (objectName.includes("_Fifth")) {
    return false;
  }
  
  // Check if the object name includes any of the names in objectsWithIntroAnimations
  ...
  glb.scene.traverse((child) => {
    if (child.isMesh) {
      ...
      if (child.name.includes("Hanging_Plank_1")) {
        ...
        workBtn = child;
        // Don't set scale to 0 if it has _Fifth in the name (already initialized at full scale)
        if (!child.name.includes("_Fifth")) {
          child.scale.set(0, 0, 0);
        } else {
          // Store the current scale as initialScale for _Fifth buttons
          child.userData.initialScale = new THREE.Vector3().copy(child.scale);
        }
      ...
      if (child.name.includes("Water")) {
        child.material = new THREE.MeshBasicMaterial({
          color: 0x8c6d4b, // Changed from blue to brown-tinted water
          transparent: true,
        ...
        // Center the video texture on the screen
        if (child.material.map) {
          child.material.map.center.set(0.5, 0.5);
          child.material.map.offset.set(0, 0);
        }
      } else {
        // Handle both naming conventions: "First", "Second", etc. and "_First", "_Second", etc.
        let materialAssigned = false;
        
        // First try the direct key matching (original behavior)
        Object.keys(textureMap).forEach((key) => {
          if (child.name.includes(key) && !materialAssigned) {
        ...
        // If no material was assigned, try the _Key pattern (for objects named with _Fifth, etc.)
        if (!materialAssigned) {
          if (child.name.includes("_First")) {
            child.material = roomMaterials.First;
            materialAssigned = true;
          } else if (child.name.includes("_Second")) {
            child.material = roomMaterials.Second;
            materialAssigned = true;
          } else if (child.name.includes("_Third")) {
            child.material = roomMaterials.Third;
            materialAssigned = true;
          } else if (child.name.includes("_Fourth")) {
            child.material = roomMaterials.Fourth;
            materialAssigned = true;
          } else if (child.name.includes("_Fifth")) {
            child.material = roomMaterials.Fifth;
            materialAssigned = true;
          }
          
          // Handle fans with the _Key pattern
          if (materialAssigned && child.name.includes("Fan")) {
            if (
              child.name.includes("Fan_2") ||
              child.name.includes("Fan_4")
            ) {
...
function handleRaycasterInteraction() {
  if (currentIntersects.length > 0) {
    ...
    Object.entries(pianoKeyMap).forEach(([keyName, soundKey]) => {
      if (object.name.includes(keyName)) {
        ...
        gsap.to(object.rotation, {
          x: object.userData.initialRotation.x + Math.PI / 60, // Much smaller rotation
    ...
    // Handle both naming conventions for buttons
    if (object.name.includes("Work_Button") || object.name.includes("My_Work_Button")) {
...
function playHoverAnimation(objectHitbox, isHovering) {
  // Reduce the scale factor for less dramatic animations
  let scale = 1.15; // Reduced from 1.4
  ...
  gsap.killTweensOf(object.position);
    ...
    if (isHovering) {
      gsap.to(smoke.scale, {
        x: 1.15, // Reduced from 1.4
        y: 1.15, // Reduced from 1.4
        z: 1.15, // Reduced from 1.4
  ...
  if (isHovering) {
    // Scale animation for all objects - keep the same scale but make it smoother
    ...
    // Handle both naming conventions for About_Button
    if (object.name.includes("About_Button")) {
      gsap.to(object.rotation, {
        x: object.userData.initialRotation.x - Math.PI / 30, // Much smaller rotation
        duration: 0.4, // Slightly faster
        ease: "power2.out", // Less bouncy animation
      });
    } else if (
    ...
    if (object.name.includes("Boba") || object.name.includes("Name_Letter")) {
      gsap.to(object.position, {
        y: object.userData.initialPosition.y + 0.1, // Reduced movement
        duration: 0.4, // Slightly faster
        ease: "power2.out", // Less bouncy animation
      });
    }
...
// Theme functionality removed - using brown/cream theme only
